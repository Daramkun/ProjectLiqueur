	...
	internal sealed class DeflateManager
	{
		...
		internal int SetDictionary ( byte [] dictionary )
		{
			...
			if ( dictionary == null || status != INIT_STATE )
				throw new CompressionProcessException ( "Stream error." );
		...
		internal int Deflate ( FlushType flush )
		{
				...
				( status == FINISH_STATE && flush != FlushType.Finish ) )
			{
				...
				throw new CompressionProcessException ( String.Format ( "Something is fishy. [{0}]", _codec.Message ) );
			}
			if ( _codec.AvailableBytesOut == 0 )
			{
				_codec.Message = _ErrorMessage [ ZlibConstants.Z_NEED_DICT - ( ZlibConstants.Z_BUF_ERROR ) ];
				throw new CompressionProcessException ( "OutputBuffer is full (AvailableBytesOut == 0)" );
			}
			...
			{
				_codec.Message = _ErrorMessage [ ZlibConstants.Z_NEED_DICT - ( ZlibConstants.Z_BUF_ERROR ) ];
				throw new CompressionProcessException ( "status == FINISH_STATE && _codec.AvailableBytesIn != 0" );
			}
